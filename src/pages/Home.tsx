...
import { UserRoundPlus, ChevronRight, ShieldCheck, CircleEqual, TrendingUp, Flame, MapPin, Heart, Sun, Moon } from 'lucide-react';
...
function greeting(): { text: string; Icon: typeof Sun } {
  ...
  if (h < 18) return { text: h < 12 ? 'Good morning' : 'Good afternoon', Icon: Sun };
  ...
  return (
    <div className="pb-24 md:pb-8">
      ...
      <div className="bg-gradient-to-br from-indigo-950 to-purple-950 px-6 pt-8 pb-10">
        <p className="text-textSecondary text-sm mb-1 flex items-center gap-1.5">
          <g.Icon size={14} /> {g.text}
        </p>
        <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">Find Your Perfect College</h1>
        ...
        {!hasProfile && (
          <Link to="/profile" className="inline-flex items-center gap-2 bg-cmGold text-cmNavy font-bold px-5 py-3 rounded-full text-sm">
            <UserRoundPlus size={16} /> Complete Profile
          </Link>
        )}
      </div>

      <div className="px-5 -mt-5 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-4 gap-2 mb-8">
          <StatBox Icon={ShieldCheck} count={counts.Safety} label="Safety" color="text-cmSafety" />
          <StatBox Icon={CircleEqual} count={counts.Match} label="Match" color="text-cmMatch" />
          <StatBox Icon={TrendingUp} count={counts.Reach} label="Reach" color="text-cmReach" />
          <StatBox Icon={Flame} count={counts['Highly Competitive']} label="Highly Competitive" color="text-cmHC" />
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {featured.map((c) => (
              <div key={c.id} className="bg-white text-cmNavy rounded-xl p-4 relative">
                ...
                <button className="absolute top-4 right-4 text-gray-300 hover:text-cmHC">
                  <Heart size={18} />
                </button>
...
function StatBox({ Icon, count, label, color }: { Icon: typeof ShieldCheck; count: number; label: string; color: string }) {
  return (
