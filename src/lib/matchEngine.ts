...
/**
 * Categorizes a college as Safety / Match / Reach / Highly Competitive for
 * a given student. Driven by the computed admission probability (which
 * already accounts for the school's acceptance rate as its baseline) —
 * not a separate hard override — so a strong student's real chances at a
 * moderately selective school are reflected accurately instead of being
 * lumped into "Highly Competitive" purely by the school's overall rate.
 */
export function categorize(college: College, profile: StudentProfile): CategorizedCollege {
  ...
  if (p >= 0.60 && acceptRate >= 0.20) {
    // Never call a school "Safety" if it's inherently very selective,
    // even if the formula's cap allows a relatively high number for it.
    category = 'Safety';
  } else if (p >= 0.35) {
    category = 'Match';
  } else if (p >= 0.15) {
