...
import { GraduationCap, Brain, Star, ArrowRight, Check } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col px-6 py-8">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center">
        <div className={`w-32 h-32 rounded-full bg-gradient-to-br ${current.color} bg-opacity-20 border-2 border-white/10 flex items-center justify-center mb-8`}>
          <current.Icon size={56} className={current.iconColor} />
        </div>
      ...
      <button
        ...
        className={`w-full font-bold py-4 rounded-xl text-center flex items-center justify-center gap-2 ${
          isLast ? 'bg-cmGold text-cmNavy' : 'bg-cmBlue text-white'
        }`}
      >
        {isLast ? (<>Get Started <Check size={18} /></>) : (<>Next <ArrowRight size={18} /></>)}
      </button>
    </div>
  );
}
