...
import { GraduationCap, Lock, UserRound, Loader2 } from 'lucide-react';
import { useAuth } from '../context/AuthContext';

export default function AuthScreen() {
  const { signInWithGoogle, signInWithClever, continueAsGuest } = useAuth();
  const [signingIn, setSigningIn] = useState<'google' | 'clever' | null>(null);

  function handleGoogle() {
    setSigningIn('google');
    setTimeout(signInWithGoogle, 500); // brief pause so the loading state is visible
  }
  function handleClever() {
    setSigningIn('clever');
    setTimeout(signInWithClever, 500);
  }
    ...
    <div className="min-h-screen bg-surface flex flex-col items-center justify-center px-6 py-10">
      ...
      <div className="w-full max-w-sm bg-surfaceMid border border-borderC rounded-xl p-6">
        ...
        <button
          ...
          className="w-full bg-white text-gray-800 font-bold py-3.5 rounded-md flex items-center justify-center gap-3 mb-2 disabled:opacity-70"
        >
          {signingIn === 'google' ? <Loader2 size={18} className="animate-spin" /> : <GoogleIcon />}
          {signingIn === 'google' ? 'Signing in…' : 'Continue with Google'}
        </button>

        <button
          onClick={handleClever}
          disabled={signingIn !== null}
          className="w-full bg-[#1E62D0] text-white font-bold py-3.5 rounded-md flex items-center justify-center gap-3 mb-2 disabled:opacity-70"
        >
          {signingIn === 'clever' ? <Loader2 size={18} className="animate-spin" /> : <CleverIcon />}
          {signingIn === 'clever' ? 'Signing in…' : 'Continue with Clever'}
        </button>
...
function CleverIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M7 8.5C7 6.567 8.567 5 10.5 5h3C15.433 5 17 6.567 17 8.5v0H13.5C11.567 8.5 10 10.067 10 12s1.567 3.5 3.5 3.5H17v0c0 1.933-1.567 3.5-3.5 3.5h-3C8.567 19 7 17.433 7 15.5v-7z" fill="#1E62D0"/>
    </svg>
  );
}
