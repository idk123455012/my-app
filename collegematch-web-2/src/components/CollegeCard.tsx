...
export default function CollegeCard({ item, isFavorite, onToggleFavorite }: Props) {
  ...
  return (
    <div className="rounded-md overflow-hidden border border-cmBorder bg-cmNavyMid shadow-lg shadow-black/40">
      <div className="flex items-center justify-between px-4 py-2.5 bg-cmNavyLight">
      ...
      <div className="p-4 flex flex-col gap-3.5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-bold text-cmTextPrimary leading-snug">{college.name}</h3>
            <p className="text-xs text-cmTextTertiary font-medium">{college.institutionType}</p>
          </div>
          <button
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-white/5 border border-white/10 rounded-sm py-2 px-1">
      <div className="text-xs font-bold text-cmTextPrimary">{value}</div>
      <div className="text-[9px] font-semibold text-cmTextTertiary mt-0.5">{label}</div>
    </div>
  );
}
